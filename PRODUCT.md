...
- **Wave 7 planned (pure Rust, no Python):** 10 native Rust provider scrapers — World Bank, IMF, ECB, CFTC COT, BLS, EIA, OFR, Treasury Direct, CoinGecko + 1 buffer. Closes international macro + futures positioning + sector-level labor + energy gaps. ~8-10 days.
- **Wave 8 conditional:** Rust axum sidecar mimicking OpenBB Workspace API contract. Ships polished cloud dashboards via Workspace UI without Python dependency. ~7 days. Decision gated on Wave 7 signal validation + Pursuit demo needs.
