...
- **v11.5 planned (next, ~10 days):** "The Explanations" — 22 items from 2026-05-05 video review. Dominant theme: pop-up explanations everywhere (15+ mentions in transcript). 6 sub-waves sequenced by dependency + risk: foundation (tooltip helper) → layout (header reshuffle, gauges side-by-side, Settings modal) → content (12 tooltips applied) → interactivity (aspect line hover, mouse-wheel zoom, OS notifications) → encyclopedia (new Wikipedia tab) → polish (candle word labels, loading %, strategy defaults). Validated: Wave 6 features all working in production (Sun/Moon/Rising populated, planet glyphs + tooltips, data freshness badges, sector dropdown, score widening with 6 new Optimal alerts).
- **Wave 7 deferred (post-v11.5):** 10 native Rust provider scrapers — World Bank, IMF, ECB, CFTC COT, BLS, EIA, OFR, Treasury Direct, CoinGecko + 1 buffer. ~8-10 days.
- **Wave 8 conditional:** Rust axum sidecar mimicking OpenBB Workspace API contract. ~7 days. Decision gated on v11.5 ship + Pursuit demo needs.
