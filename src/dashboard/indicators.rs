...
// ---------------------------------------------------------------------------
// Per-ticker composite score (zero extra DB calls — uses loaded indicators)
// ---------------------------------------------------------------------------
...
pub fn compute_ticker_score(
    ...
    // MACD histogram: ±0.2% of price maps to full range
    ...
    let macd_score = if latest > 0.0 {
        let hist = macd_v - signal_v;
        ((hist / latest * 500.0) + 50.0).clamp(0.0, 100.0)
    } else {
        50.0
    };

    // AV sentiment: -1..+1 → 0..100
...
// ---------------------------------------------------------------------------
// Lagrange Score — blends financial, astro, macro, and short squeeze signals
// ---------------------------------------------------------------------------
...
pub fn compute_lagrange_score(
    ...
    // Component 1: Financial (35%) — reuse existing ticker score
    let (fin_score, _) = compute_ticker_score(indicators, rows, sentiment);

    // Component 2: Astrology (25%) — today's astro_score (0-100)
    let astro = astro_score
        .as_ref()
        .and_then(|s| s.astro_score.map(|v| v as f32))
        .unwrap_or(50.0);

    // Component 3: Macro environment (25%)
    //   VIX: calm markets bullish. VIX 10→90pts, VIX 30→50pts, VIX 80+→0pts
    //   Yield spread (T10Y2Y): positive = normal curve = bullish
    ...
    // Component 4: Short squeeze signal (15%)
    //   High short % + rising price = squeeze setup (bullish catalyst)
    let short_score = short_interest.as_ref().and_then(|si| {
        ...
        let price_rising = rows.len() >= 5 && {
            let latest = rows[0].close.to_string().parse::<f32>().unwrap_or(0.0);
