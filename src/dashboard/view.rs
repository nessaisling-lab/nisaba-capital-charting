...
impl Dashboard {
    pub fn view(&self) -> Element<'_, Message> {
        ...
        let macro_strip_intl = row![
            text(macro_fmt("Euribor 3M", "DBNOMICS:ECB/FM/M.U2.EUR.RT.MM.EURIBOR3MD_.HSTA", "%", "")).size(theme::text_base()),
            text(macro_fmt("PBoC", "DBNOMICS:BIS/WS_CBPOL/M.CN", "%", "")).size(theme::text_base()),
            text(macro_fmt("EU CPI", "DBNOMICS:Eurostat/prc_hicp_manr/M.RCH_A.CP00.EA", "%", "")).size(theme::text_base()),
            text(macro_fmt("OECD CLI", "DBNOMICS:OECD/MEI_CLI/LOLITOAA.USA.M", "", "")).size(theme::text_base()),
            text(macro_fmt("Credit/GDP", "DBNOMICS:BIS/WS_TC/Q.US.P.A.M.770.A", "%", "")).size(theme::text_base()),
        ].spacing(20);
                    ...
                    let pm_items: Vec<Element<Message>> = self.polymarket.iter().take(8).map(|m| {
                        ...
                        let vol = m.volume.as_ref()
                            .map(|d| {
                                ...
                                if v >= 999_500.0 { format!("${:.1}M", v / 1_000_000.0) }
