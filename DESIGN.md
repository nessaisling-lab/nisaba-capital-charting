...
**Current version:** v13.1 "Polish corrections" (shipped 2026-05-06)
**Next milestones:**
- **v13.2 "Polish wave 2"** (planned) — Font-scale-aware gutter for compact_nav (the v13.1.7 16px static fix isn't enough at Compact size — gutter overlap shrinks/grows with font scale), forecast content expansion, Research tab tooltips, Universe column tooltip re-verify.
- **v13.1 "Polish corrections"** (shipped 2026-05-06) — 8 sub-items: forecast color revert + 5-band calendar palette, Lifecycle ink contrast, partial scrollbar gutter (tab strip + compact_nav 16px), Canvas BellIcon Option C with rocking ring, inline shooting star at fixed 18px, Astrology layout Shrink+Fill revert, header gutter (partial). User v97: *"Overall huge improvement. Everything's on with exceptions of the lack of consistency in regards to the gutter boundary."*
- **v13.0 "Polish & Performance"** (shipped 2026-05-06) — 8 fixes from video v95: Lifecycle cache (~6 Swiss Eph calls/render → 0, root cause of Astrology lag), wheel/transits FillPortion split, bell glyph size up, Year-of-Lord tooltip with plain-English explanation, persistent notification counter badge, toast expiry on animation ticks. User v96 follow-up confirmed perf restored: *"the performance on the astrology tab is going really well. Perfect."*
- **Wave 9.5 "UI integration"** (shipped 2026-05-06) — Surfaced every Wave 9 engine layer in the Astrology tab. New Year-of-Lord badge, planet-hover enrichment (decan/Sabian/critical/OOB), Lifecycle section (Solar Return + return countdowns + progressed Sun), Backtest TimeWindow picker. New `fetch_ipo_date()` query + `state.natal_ipo_date` plumbing. 9.5.5 (progressed Sun ingress pill) deferred.
- **Wave 9 "The Compounding"** (shipped 2026-05-06) — Production-tier financial astrology engine. 9 new modules + 5 paired sub-waves: declination/aspect-gradient (Foundation), Solar Return + Decans (Year), Planetary Returns + Sabian Symbols (Cycle), Profections + Critical Degrees + OOB (Lord), Progressions + Backtest extension (Maturation). 132/132 lib tests + 8/8 backtest tests pass. AAPL chart (IPO 1980-12-12 09:30 EST) validates: Year of Venus (10th house · Libra), Saturn return 2010, progressed Sun ~6° Aquarius, Sabian "child and dog wearing borrowed eyeglasses." Engine answers "where in the corporate lifecycle is today happening" — not just "what aspects fire today."
...
- **v12.0.C** (shipped 2026-05-06) — Munger narrative expanded combinatorially. 11 sectors with aphorism quotes, 5-band ROE, full debt/EV/FCF/news-tone coverage, 6-band score closer. Voice preserved (mental models, inversion, terse wit).
- **v12.0.D** (shipped 2026-05-06) — Chart hover state-mutation skip on intra-bar moves. Iced 0.14 redraws on canvas state mutation alone — v11.7.D bar-snap missed this half. Fix: skip `*state = new_pos` when bar unchanged.
- **v11.9 "The Convergence"** (shipped 2026-05-05) — final layout converged after 5 iteration rounds: toast overlay via `stack!` not `column!` (fixes top-pushes-down bug), fetching pill with ShootingStar canvas in tab strip chrome, alert pill alongside with 8s TTL, gear icon as sole Settings entry point, loading bar inline with tip-star. Tab::Settings retained for view dispatch but dropped from `Tab::all()`. OS notifications parked (installer-deferred — HRESULT 0x80070005 persists for unpackaged Rust apps despite AUMID + shortcut registration).
- **v11.8 "The Persistence"** (shipped 2026-05-05) — 9 sub-waves: A settings prominence, B header polish, C Paper Trail icon TARGET, D Windows AUMID registration, E loading-bar drop (later reverted), F ShootingStar animation, G Wikipedia per-ticker fetch, H loading-bar restore + popup-pill drop, I gear-only at tab strip right.
- **v11.7 "The Resolution"** (shipped 2026-05-05) — 6 sub-waves: A Paper Trail icon mockup (RECEIPT picked, later replaced by TARGET in v11.8.C), B Munger Sell/StrongSell phrase variance, C OS notifications diagnostic logging + Test Notification button, D chart hover bar-index snap (1000Hz mouse → ~50 redraws per pass), E+F sparkle re-seed slowed 8Hz→1.5Hz with sinusoidal alpha pulse.
- **v11.6 "The Persistence"** (shipped 2026-05-05) — 9 sub-waves: A header redo (4 mockup iterations), B-K council de-astro / natal sphere / 3-month calendar / Lagrange polish / sparkle upgrade / gauge clarity / fetch timeout / canvas Cache. Council de-astro validated by user.
